...
export default function RconConsole({
  ...
  const [logs, setLogs] = useState<LogLine[]>([
    { type: "system", text: `Connecting to ${serverTitle}…` },
  ]);
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (msg.status === "connected") {
          setConnected(true);
          addLog("system", `✅ Connected to ${msg.server}`);
        } else if (msg.output) {
        ...
        addLog("output", event.data as string);
      }
    };
      ...
      <div className="w-full max-w-3xl glass-card rounded-xl overflow-hidden shadow-2xl animate-fade-in">
        ...
        <div className="terminal h-80 overflow-y-auto p-4 space-y-1 font-mono text-sm">
          {logs.map((line, i) => (
            <div
              ...
              className={cn(
                "leading-relaxed",
                line.type === "input" && "text-neon-blue",
        ...
        <div className="flex items-center gap-2 px-4 py-3 border-t border-white/10 bg-black/40">
          ...
          <input
            ...
            disabled={!connected}
            placeholder={connected ? "Enter RCON command…" : "Waiting for connection…"}
          ...
          <button
            ...
            className="text-neon-green hover:text-neon-green/70 transition-colors disabled:opacity-30"
          >
